...
for(i in 1:1000){
  ...
  leg = read.table(paste0(dir, "RAREsim2/datasets/Hapgen", Nsim/1000, "K_pruned2/", Pop, "/Round1/chr19.block37.", Pop, ".sim", i, ".", Nsim, ".", p.case[1], "fun.100syn.legend"), header=T) %>%
    mutate(gene=ifelse(gene=="ZNF333;ZNF333(NM_001352243:exon9:UTR5)", "ZNF333", gene))
  leg$row = 1:nrow(leg)
  
  # read in the haplotype files
  cases.hap = fread(paste0(dir, "RAREsim2/datasets/Cases2/", Pop, "/Round1/chr19.block37.", Pop, ".sim", i, ".", Ncase, ".power.cases.same.", p.case[j], "fun.100syn.haps-sample.gz"))
  cases.hap = as.data.frame(cases.hap)
  
  controls.hap = fread(paste0(dir, "RAREsim2/datasets/Controls2/", Pop, "/Round1/chr19.block37.", Pop, ".sim", i, ".", Ncase, ".controls.same.100fun.100syn.110.haps-remainder.gz"))
...
write.table(out.genes.all.p, paste0(dir, "RAREsim2/results/", Pop, "/power_same_pvalues2_", Pop, "_", p.case[j], ".txt"), quote=F, row.names=F, col.names=T)
...
write.table(power.genes.out, paste0(dir, "RAREsim2/results/", Pop, "/power_same_results2_", Pop, "_", p.case[j], ".txt"), quote=F, row.names=F, col.names=T)
