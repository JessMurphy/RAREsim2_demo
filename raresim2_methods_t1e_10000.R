...
#end = (1+id)*1000 - (pop.index-1)*9000
end = id - (pop.index-1)*10000
start = end - 999
print(paste0("Batch: ", end))
...
for(i in start:end){
  ...
  if (n==1){ 
     ...
     leg = read.table(paste0(dir, "RAREsim2/datasets/Hapgen", Nsim/1000, "K_pruned2/", Pop, "/Round1/chr19.block37.", Pop, ".sim", i, ".", Nsim, ".", p.case1, "fun.100syn.legend"), header=T) %>%
       mutate(gene=ifelse(gene=="ZNF333;ZNF333(NM_001352243:exon9:UTR5)", "ZNF333", gene))
     leg$row = 1:nrow(leg)

     # read in the haplotype files
     cases.hap = fread(paste0(dir, "RAREsim2/datasets/Cases2/", Pop, "/Round", n, "/chr19.block37.", Pop, ".sim", i, ".", Ncase, ".t1e.cases.100fun.100syn.", p.case2, ".haps-sample.gz"))
     cases.hap = as.data.frame(cases.hap)
  
     controls.hap = fread(paste0(dir, "RAREsim2/datasets/Controls2/", Pop, "/Round", n, "/chr19.block37.", Pop, ".sim", i, ".", Ncase, ".controls.same.100fun.100syn.", p.case2, ".haps-remainder.gz"))
     controls.hap = as.data.frame(controls.hap)

  } else {

     # read in the legend file
     leg = read.table(paste0(dir, "RAREsim2/datasets/Hapgen", Nsim/1000, "K_pruned2/", Pop, "/Round", n, "/chr19.block37.", Pop, ".sim", i, ".", Nsim, ".100fun.100syn.legend"), header=T) %>%
       mutate(gene=ifelse(gene=="ZNF333;ZNF333(NM_001352243:exon9:UTR5)", "ZNF333", gene))
     leg$row = 1:nrow(leg)

     # read in the haplotype files
     cases.hap = fread(paste0(dir, "RAREsim2/datasets/Cases2/", Pop, "/Round", n, "/chr19.block37.", Pop, ".sim", i, ".", Ncase, ".t1e.cases.100fun.100syn.haps-sample.gz"))
     cases.hap = as.data.frame(cases.hap)
  
     controls.hap = fread(paste0(dir, "RAREsim2/datasets/Controls2/", Pop, "/Round", n, "/chr19.block37.", Pop, ".sim", i, ".", Ncase, ".controls.same.100fun.100syn.haps-remainder.gz"))
     controls.hap = as.data.frame(controls.hap)
  }
...
write.table(out.genes.all.p, paste0(dir, "RAREsim2/results/", Pop, "/t1e_pvalues2_", Pop, "_", end, ".txt"), quote=F, row.names=F, col.names=T)
...
write.table(power.genes.out, paste0(dir, "RAREsim2/results/", Pop, "/t1e_results2_", Pop, "_", end, ".txt"), quote=F, row.names=F, col.names=T)
